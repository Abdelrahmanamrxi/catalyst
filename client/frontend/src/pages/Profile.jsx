...
import { useEffect,useState,memo,useRef, Suspense } from 'react'
import { IoIosArrowForward } from "react-icons/io";
import { IoIosArrowDown } from "react-icons/io";
import { IoMdArrowRoundBack } from "react-icons/io";
import user from '../assets/user.png'
import { Logout,DecodeJWT } from '../utility/functions';
 ...
 function Profile() {
    ...
    const [prev_order,set_order]=useState(false)
    const[privacy,set_privacy]=useState(false)
    const[coupon,set_coupon]=useState(false)
    ...
    async function UpdatePassword(e){
        ...
        set_message('')
        try{
        const response=await axios.patch('http://localhost:5000/api/users/profile',{current_password,new_password,email:profile.email},{headers:{
...
we do not accept refunds or exchanges. All sales are final. We encourage customers to carefully review product descriptions, sizes, and other details before making a purchase. If you encounter an issue with your order, please contact us at , and we will do our best to assist you.
          ...
          <div onClick={ToggledCoupon} className='cursor-pointer flex flex-row items-center gap-3'><h1 className='md:text-2xl text-xl font-serif font-bold'>Coupons</h1>
          {coupon?<h1><IoIosArrowDown/></h1>:<h1> <IoIosArrowForward/></h1>}
          </div>
          {coupon?<div data-aos="fade-up"  className=' h-15 flex-row flex gap-5'><h1 className='font-extrabold text-lg font-serif'>8DE312</h1>
          <h2 className='font-bold  text-md font-serif '>Use this coupon and GET 10% OFF</h2>
          </div>:null}
         <div className='h-24 '> <button onClick={()=>{Logout(navigate)}} className='mt-10 bg-black flex text-white justify-center w-32 rounded-sm font-semibold px-2 py-1 font-serif  '>Sign Out</button></div>
